...
LIST     * darea_list = NULL;     /* the area list of loaded areas     */
sqlite3  * db = NULL;             /* the connection to the sqlite3 db  */
time_t current_time;
...
int main(int argc, char **argv)
{
  ...
  if( rc ){
	log_string("Can't open database!");
  ...
  switch(dsock->state)
  {
    ...
    case STATE_VERIFY_PASSWORD:
      if (!strcmp(crypt(arg, dsock->player->name), dsock->player->password))
      {
        ...
        /* put him in the start room */
        dsock->player->room = find_room(START_ROOM);
    ...
    case STATE_ASK_PASSWORD:
      ...
      if (!strcmp(crypt(arg, dsock->player->name), dsock->player->password))
      {
        ...
        else
        {
          ...
          AttachToList(p_new, p_new->room->mobiles);
